...
<section class="bg-[url('../../public/bg-waves-white.svg')] dark:bg-[url('../../public/bg-waves.svg')] bg-cover min-h-screen w-full pt-32 selection:bg-green-600 selection:text-white max-lg:pb-10 max-2xl:pb-10">
    <div class="py-8 lg:py-16 px-4 mx-auto max-lg:w-[80%] max-w-screen-md max-h-screen rounded-xl shadow-2xl shadow-primary-900 backdrop-blur-xl">
        <h2 class="mb-4 text-4xl tracking-tight font-extrabold text-center text-gray-900 dark:text-white">PDF Explainer</h2>
        <p class="mb-8 lg:mb-16 font-light text-center text-gray-500 dark:text-gray-400 sm:text-xl">Vous perdez du temps à lire des pdf en entier pour prendre des notes ? Insérez votre fichier et cliquez sur générer pour obtenir un résumé précis en quelques secondes.</p>
        <form action="{{ route('pdf_to_text') }}" method="POST" class="space-y-8" enctype="multipart/form-data">
            @csrf
            @if ($errors->any())
            ...
            <div class="flex flex-auto m-0 h-11">
                <input type="file" name="pdf" id="pdf" class="block mr-3 w-full border border-gray-200 shadow-sm rounded-md text-sm focus:z-10 focus:border-blue-500 focus:ring-blue-500 dark:bg-slate-900 dark:border-gray-700 dark:text-gray-400 file:bg-transparent file:border-0 file:bg-gray-100 file:mr-4 file:py-3 file:px-4 dark:file:bg-gray-700 dark:file:text-gray-400">
